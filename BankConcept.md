...
# individual credit reporting

## 术语

### 借贷交易

借贷的范畴，除各种形式的贷款、信用卡透支以外，也包括以各种其他名义融资但实质上是借贷的行为，即融资方负有明确还款责任的交易。常见的产品种类包括:贷款、信用卡、融资租赁等。

### 个人借贷账户

个人借贷交易信息是以借贷账户为中心进行描述的。

为了记录借款人不同的借贷交易行为，征信系统抽象出 6 类不同的借贷账户，每类账户 的属性及描述对象不同。6 类账户如下:

- ◼  非循环贷账户(**D1**):用于描述非循环贷款，包括一次性放款和分次放款。
- ◼  循环贷账户(**R1**):用于描述循环额度下还款统一管理(即每月根据当前累计借 款余额计算出一个还款金额通知借款人、借款人统一归还)的贷款，也可用于汇总报送循环授信额度下逐笔管理的所有贷款。
- ◼  贷记卡账户(**R2**):用于描述借款人使用贷记卡类产品的透支及还款行为。
- ◼  准贷记卡账户(**R3**):用于描述借款人使用准贷记卡类产品的透支及还款行为。
- ◼  循环额度下分账户(**R4**):用于描述循环授信额度下逐笔管理(即需要针对每一笔借款分开归还)的贷款。
- ◼  催收账户(**C1**):用于描述剩余本息均处于催收状态(即债权人要求债务人尽快归还全部借款)的借款。



### 金额的约定

一般情况下，个人信用报告中金额类(例如额度、金额等)数据项的取值均已统一折算为人民币金额，单位为元，精确到个位数。例外情况是，当个人作为企业相关还款责任人时，“还款责任金额”的取值是实际币种对应的金额;对于外币计价的“还款责任金额”，在信息概要中进行汇总展示时，征信系统会先按照查询信用报告时的汇率折算为人民币。



### 出现次数约定

本规范中采用以下符号说明信息单元、信息段以及数据项的出现次数:

-   “0:N”表示不出现或最多出现N次。
-   “0:*”表示可以不出现、也可以出现多次。
-   “1:1”表示出现且仅出现1次。
-   “→”符号标识数据项可以重复出现，且必须同时重复出现，出现次数应与表示出 现重复次数的数据项取值保持一致，若出现次数为 0，则后面带“→”的数据项均 不出现。



## 报告组成结构

个人信用报告在本质上是一个数据集，包括 **8 大类信息、24 个数据块、24 个信息单元**。
...
一份 XML 形式信用报告中的所有信息包含在根节点 Document 中。信用报告数据组织采用的四层结构(自上至下依次数据块、信息单元、信息段、数据项)对应的元素都有相应的 XML 标签(详见附录)。上一层元素标签包含下一层元素标签，形成结构化的信用报告。 



## XML描述

一份信用报告 XML 的根节点标签为 Document。根节点下有 5 类节点:

-  数据块节点:数据块是构成个人信用报告的基本组件的集合，该类节点在信用报告中必出现;
-  信息单元节点:信息单元为数据块的下级元素，当数据块非空时，信息单元节点必须出现，并且可出现多次;
-  信息段节点:信息段为信息单元的下级元素，信息段节点在信息单元中最多出现一次，特定场景下部分信息段节点不出现;
-  组合数据项节点:组合数据项为信息段或者结构简单的信息单元的下级元素，组合数据项由一组可重复出现的数据项构成;
-  简单数据项节点:简单数据项为信息段、组合数据项、以及结构简单的信息单元的下级元素。




上述 5 类节点的标签通过位数进行区分，各层级标签位数依次为:数据块节点 3 位、信息单元节点 4 位、信息段节点 5 位、组合数据项节点 6 位(最后一位为“H”)、简单数据项节点8位

下列表格中“XML Tag”为各节点的标签名;出现次数“[x..y]”表示，该节点最少出现 x 次，最多出现 y 次。例如，[1..10]表示该项最少出现 1 次，最多出现10 次。
